...
#
# ВАЖНО - ограничение подхода: API отдаёт только активные брони "как есть
...
#
# Название поля даты создания брони в ответе TravelLine PMS API нужно
# проверить на реальных данных - ниже перебирается несколько вероятных
# вариантов имени поля. Если ни одно не подошло, бронь пропускается и
# считается в missing_created_at (в отчёте будет предупреждение).
...
def _get_reservation_created_at(reservation):
    """Пытается достать дату создания брони из одного из вероятных полей."""
    for key in _CREATED_AT_FIELDS:
...
def get_pickup_snapshot(token, property_id, week_start, week_end, created_before):
    ...
    for number in numbers:
        ...
        created_at = _get_reservation_created_at(reservation)
    ...
    for weeks_ahead in weeks_ahead_list:
        ...
        if missing_now or missing_ly:
            warnings.append(
                ...
                f"{missing_now + missing_ly} броней - проверьте название поля даты создания "
                f"в ответе TravelLine API (см. _CREATED_AT_FIELDS)."
            )
...
def main():
    ...
    full_report = "\n\n".join([yesterday_section, yoy_section, room_type_section, pace_section])
